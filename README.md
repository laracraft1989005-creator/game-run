...
- **公交车骑乘奖励阶段**
  - 弹射板触发时 60% 概率召唤公交车
  - 公交车从右侧驶入，玩家跳上车顶（Y=2.8）
  - 车顶 4 秒奖励关：3 车道 × 8 排 = 24 个密集金币
  - 骑乘期间跳过地面障碍碰撞，正常 3 车道操作
  - 结束后公交车向左驶离，玩家平滑降落回地面
- **动态地面系统**：PlayerController 支持可变地面高度（`_groundY`）
- **程序化公交车模型**：车身 + 车顶 + 窗户 + 车轮，零 GLB 依赖
- **"BUS RIDE!" UI 提示** + 公交喇叭/引擎音效
